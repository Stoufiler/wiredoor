...
export const validateDomainArray = async (
  domains: string[],
  fieldName: 'allowedDomains' | 'blockedDomains' = 'allowedDomains',
): Promise<string[]> => {
  ...
  for (const domain of domains) {
    ...
    try {
      await dnsValidator.validateDomainWithCache(domain);
    } catch {
...
export const httpServiceValidator: ObjectSchema<HttpServiceType> = Joi.object({
  ...
  allowedDomains: Joi.array()
    ...
    .allow(null)
    .external((domains) => validateDomainArray(domains, 'allowedDomains'))
    .optional(),
  blockedDomains: Joi.array()
    .items(Joi.string().domain().optional())
    .allow(null)
    .external((domains) => validateDomainArray(domains, 'blockedDomains'))
    .optional(),
